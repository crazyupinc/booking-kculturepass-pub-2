import { FunctionComponent, useState, useCallback, useEffect } from "react";
import LoginPopup from "../components/login-popup";
import PortalPopup from "../components/portal-popup";
import { useNavigate } from "react-router-dom";
import DrawerPopUp from "../components/drawer-pop-up";
import PortalDrawer from "../components/portal-drawer";
import styles from "./main.module.css";
const Main: FunctionComponent = () => {
  const [isLoginPopupOpen, setLoginPopupOpen] = useState(false);
  const navigate = useNavigate();
  const [isDrawerPopUpOpen, setDrawerPopUpOpen] = useState(false);

  const openLoginPopup = useCallback(() => {
    setLoginPopupOpen(true);
  }, []);

  const closeLoginPopup = useCallback(() => {
    setLoginPopupOpen(false);
  }, []);

  const onIntroClick = useCallback(() => {
    navigate("/intro");
  }, [navigate]);

  useEffect(() => {
    const scrollAnimElements = document.querySelectorAll(
      "[data-animate-on-scroll]"
    );
    const observer = new IntersectionObserver(
      (entries) => {
        for (const entry of entries) {
          if (entry.isIntersecting || entry.intersectionRatio > 0) {
            const targetElement = entry.target;
            targetElement.classList.add(styles.animate);
            observer.unobserve(targetElement);
          }
        }
      },
      {
        threshold: 0.15,
      }
    );

    for (let i = 0; i < scrollAnimElements.length; i++) {
      observer.observe(scrollAnimElements[i]);
    }

    return () => {
      for (let i = 0; i < scrollAnimElements.length; i++) {
        observer.unobserve(scrollAnimElements[i]);
      }
    };
  }, []);

  const onConcertClick = useCallback(() => {
    navigate("/eventconcert");
  }, [navigate]);

  const onFestivalClick = useCallback(() => {
    navigate("/eventfestival");
  }, [navigate]);

  const onExhibitionExperienceClick = useCallback(() => {
    navigate("/eventexhibitionexperience");
  }, [navigate]);

  const onEventTextClick = useCallback(() => {
    navigate("/eventconcert");
  }, [navigate]);

  const onCustomerServiceTextClick = useCallback(() => {
    navigate("/customerservicenotice");
  }, [navigate]);

  const openDrawerPopUp = useCallback(() => {
    setDrawerPopUpOpen(true);
  }, []);

  const closeDrawerPopUp = useCallback(() => {
    setDrawerPopUpOpen(false);
  }, []);

  const onLandingButtonClick = useCallback(() => {
    navigate("/eventfestival");
  }, [navigate]);

  const onButton2Click = useCallback(() => {
    navigate("/eventconcert");
  }, [navigate]);

  const onProductCard1Click = useCallback(() => {
    navigate("/eventdetail");
  }, [navigate]);

  const onArtistCardClick = useCallback(() => {
    navigate("/myartisthome");
  }, [navigate]);

  const onLinkText5Click = useCallback(() => {
    navigate("/termsconditions");
  }, [navigate]);

  const onLinkText6Click = useCallback(() => {
    navigate("/personalinformationpolicy");
  }, [navigate]);

  const onInstaClick = useCallback(() => {
    window.open("https://www.instagram.com/");
  }, []);

  const onFaceBookClick = useCallback(() => {
    window.open("https://www.facebook.com/");
  }, []);

  const onPinterClick = useCallback(() => {
    window.open("https://www.pinterest.co.kr/");
  }, []);

  return (
    <>
      <div className={styles.main}>
        <img className={styles.imgIcon} alt="" src="/img@2x.png" />
        <div className={styles.greyFilter} />
        <div className={styles.header}>
          <nav className={styles.naviDiv}>
            <header className={styles.headerNavigation}>
              <div className={styles.logo}>
                <img
                  className={styles.logoBackgroundIcon}
                  alt=""
                  src="/logobackground@2x.png"
                />
                <div className={styles.kCulturePass}>K-Culture Pass</div>
              </div>
              <div className={styles.navigationMenu}>
                <a className={styles.navigationLink}>
                  <h2 className={styles.intro} onClick={onIntroClick}>
                    About
                  </h2>
                  <div className={styles.event}>
                    <div
                      className={styles.memberSectionSubmenu}
                      data-animate-on-scroll
                    >
                      <img
                        className={styles.frameIcon}
                        alt=""
                        src="/frame.svg"
                      />
                      <div className={styles.menuList}>
                        <button
                          className={styles.concert}
                          onClick={onConcertClick}
                        >
                          Concert
                        </button>
                        <button
                          className={styles.concert}
                          onClick={onFestivalClick}
                        >
                          Festival
                        </button>
                        <button
                          className={styles.exhibitionExperience}
                          onClick={onExhibitionExperienceClick}
                        >
                          <p className={styles.exhibition}>Exhibition</p>
                          <p className={styles.exhibition}>Experience</p>
                        </button>
                      </div>
                    </div>
                    <b className={styles.event1} onClick={onEventTextClick}>
                      Event
                    </b>
                  </div>
                  <b
                    className={styles.customerService}
                    onClick={onCustomerServiceTextClick}
                  >
                    Customer service
                  </b>
                </a>
                <div className={styles.memberSection} onClick={openLoginPopup}>
                  <button className={styles.button}>
                    <div className={styles.register}>Sign in</div>
                  </button>
                  <button className={styles.button1}>
                    <div className={styles.logIn}>Log in</div>
                  </button>
                </div>
              </div>
              <img
                className={styles.hambugerMenuIcon}
                alt=""
                src="/hambugermenu.svg"
                onClick={openDrawerPopUp}
              />
            </header>
          </nav>
          <div className={styles.hero}>
            <div className={styles.title}>
              <b className={styles.text}>한 눈에 보는 KPOP Festival</b>
              <b className={styles.text1}>
                <span>K-Culture</span>
                <span className={styles.span}> </span>
                <span className={styles.pass}>Pass</span>
              </b>
              <b
                className={styles.text2}
              >{`K-컬처 패스를 통해 일상의 회복을 즐기세요. `}</b>
            </div>
            <button
              className={styles.landingButton}
              onClick={onLandingButtonClick}
            >
              <div className={styles.seeProduct}>상품보기</div>
            </button>
          </div>
        </div>
        <div className={styles.contentProducts}>
          <div className={styles.products}>
            <div className={styles.productCategory}>
              <div className={styles.div}>
                <div className={styles.title1}>
                  <div className={styles.concerts}>K-Culture Pass Concerts</div>
                  <button className={styles.button2} onClick={onButton2Click}>
                    <div className={styles.seeAllConcerts}>콘서트 전체보기</div>
                    <img
                      className={styles.arrowRightIcon}
                      alt=""
                      src="/arrowright.svg"
                    />
                  </button>
                </div>
                <div className={styles.subTitle}>
                  <div className={styles.nowAvailable}>
                    Now available on K-Culture Pass !
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.productList}>
              <div className={styles.div1}>
                <div
                  className={styles.productCard1}
                  onClick={onProductCard1Click}
                >
                  <img className={styles.imgIcon1} alt="" src="/img1@2x.png" />
                  <div className={styles.ticketInfo}>
                    <div className={styles.name}>SEVENTEEN</div>
                    <div className={styles.place}>FANMEETING'LOVE'</div>
                    <div className={styles.place}>in JAPAN</div>
                  </div>
                </div>
                <div className={styles.productCard2}>
                  <img className={styles.imgIcon1} alt="" src="/img2@2x.png" />
                  <div className={styles.ticketInfo1}>
                    <div className={styles.name1}>Forrestella | The Light</div>
                    <div className={styles.place}>Forrestella Tour Concert</div>
                    <div className={styles.place}>in Seoul</div>
                  </div>
                </div>
              </div>
              <div className={styles.div1}>
                <div className={styles.productCard3}>
                  <img
                    className={styles.imgIcon3}
                    alt=""
                    src="/image-6@2x.png"
                  />
                  <div className={styles.ticketInfo1}>
                    <div className={styles.name1}>SUGA | Agust D</div>
                    <div
                      className={styles.place}
                    >{`SUGA | Agust D TOUR ‘D-DAY’ `}</div>
                    <div className={styles.place}>in JAPAN</div>
                  </div>
                </div>
                <div className={styles.productCard3}>
                  <img
                    className={styles.imgIcon1}
                    alt=""
                    src="/image-72@2x.png"
                  />
                  <div className={styles.ticketInfo1}>
                    <div className={styles.name1}>Forrestella | The Light</div>
                    <div className={styles.place}>Forrestella Tour Concert</div>
                    <div className={styles.place}>in Seoul</div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className={styles.contentProducts1}>
          <div className={styles.titleSection}>
            <div className={styles.title2}>
              <b className={styles.seeAllProduct}>
                새로운 아티스트를 만나보세요!
              </b>
            </div>
          </div>
          <div className={styles.artists}>
            <div className={styles.artistListRow1}>
              <div className={styles.div3}>
                <div className={styles.artistCard} onClick={onArtistCardClick}>
                  <img className={styles.imgIcon5} alt="" src="/-1@2x.png" />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>BLACKPINK</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--110@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>HENRY</div>
                  </div>
                </div>
              </div>
              <div className={styles.div3}>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--111@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>ONEUS</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--109@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>KINGDOM</div>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.artistListRow2}>
              <div className={styles.div3}>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--113@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>E’LAST</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1101@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>STAYC</div>
                  </div>
                </div>
              </div>
              <div className={styles.div3}>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1102@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>KIM SEJEONG</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1091@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>Dreamcatcher</div>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.artistListRow2}>
              <div className={styles.div3}>
                <div className={styles.artistCard}>
                  <img className={styles.imgIcon5} alt="" src="/-11@2x.png" />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>MIRAE</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1103@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>KIMJUNSU</div>
                  </div>
                </div>
              </div>
              <div className={styles.div3}>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1102@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>KIM SEJEONG</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1091@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>Dreamcatcher</div>
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.artistListRow2}>
              <div className={styles.div3}>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--112@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>BLACKPINK</div>
                  </div>
                </div>
                <div className={styles.artistCard}>
                  <img
                    className={styles.imgIcon6}
                    alt=""
                    src="/-20230601--1103@2x.png"
                  />
                  <div className={styles.artistName}>
                    <div className={styles.name4}>KIMJUNSU</div>
                  </div>
                </div>
              </div>
              <div className={styles.div10} />
            </div>
          </div>
        </div>
        <footer className={styles.footer}>
          <div className={styles.columns}>
            <div className={styles.columnParent}>
              <div className={styles.column}>
                <div className={styles.logo}>
                  <img
                    className={styles.logoBackgroundIcon}
                    alt=""
                    src="/logobackground1@2x.png"
                  />
                  <div className={styles.kCulturePass}>K-Culture Pass</div>
                </div>
              </div>
              <div className={styles.column1}>
                <div className={styles.footerLinks}>
                  <b className={styles.link}>{`주식회사 케이컬처패스   `}</b>
                  <div className={styles.link1}>대표 홍길동</div>
                  <div className={styles.link1}>사업자번호 123-45-67890</div>
                  <div className={styles.link3}>전화번호 070-1234-5678</div>
                  <div className={styles.link4}>
                    이메일 info@kculturepass.co.kr
                  </div>
                </div>
              </div>
            </div>
            <div className={styles.columnWrapper}>
              <div className={styles.column2}>
                <div className={styles.footerLinks1}>
                  <div className={styles.link5} onClick={onLinkText5Click}>
                    서비스이용약관
                  </div>
                  <div className={styles.link5} onClick={onLinkText6Click}>
                    개인정보취급방침
                  </div>
                  <div className={styles.place}>
                    통신판매업 신고번호 제2023-서울강동-1234호
                  </div>
                  <div className={styles.place}>
                    주소 서울특별시 송파구 송파동 송파대로 123, 7층 701-가호
                    비지니스 센터 A실 (송파빌딩)
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className={styles.copyright}>
            <div className={styles.snsParent}>
              <div className={styles.sns}>
                <button className={styles.insta} onClick={onInstaClick} />
                <button className={styles.faceBook} onClick={onFaceBookClick} />
                <button className={styles.pinter} onClick={onPinterClick} />
              </div>
              <div className={styles.licenseMark}>
                <div className={styles.license}>
                  © 2023 by Powered and secured by Crazyupinc.
                </div>
              </div>
            </div>
          </div>
        </footer>
      </div>
      {isLoginPopupOpen && (
        <PortalPopup
          overlayColor="rgba(113, 113, 113, 0.3)"
          placement="Centered"
        >
          <LoginPopup onClose={closeLoginPopup} />
        </PortalPopup>
      )}
      {isDrawerPopUpOpen && (
        <PortalDrawer
          overlayColor="rgba(113, 113, 113, 0.3)"
          placement="Right"
          onOutsideClick={closeDrawerPopUp}
        >
          <DrawerPopUp onClose={closeDrawerPopUp} />
        </PortalDrawer>
      )}
    </>
  );
};

export default Main;
